...
def hots_prom_enh_piechart(data=None):
    ...
    if not data:
        ...
        for cl in ["HepG2", "K562"]:
            ...
            cnt_prim_proms = hot_proms.intersect(PRIM_PROMOTERS_FILE, wa=True).groupby(c=4, o="collapse").count()
            cnt_proms -= cnt_prim_proms
            ...
            data.append((cnt_proms, cnt_prim_proms, cnt_intronic, cnt_intergenic))

            return data

    labels = ["promoters", "intronic", "intergenic"]
    fig1, [ax1, ax2] = plt.subplots(1, 2)
    ax1.pie(data[0], labels=labels, autopct='%2.1f%%', shadow=True, startangle=90, counterclock=False)
    ax1.axis('equal')
    ax1.set_title("HepG2")
    ax2.pie(data[1], labels=labels, autopct='%2.1f%%', shadow=True, startangle=90, counterclock=False)
    ax2.axis('equal')
    ax2.set_title("K562")

    plt.tight_layout()

    save_file = join(PLOTS_DIR, "piechart_introns_intergenic.pdf")
...
def hots_prom_enh_barplot(data=None):
    ...
    [ax.bar_label(g, label_type="center", fmt='%.1f %%', fontweight="bold", color="white") for g in gs]
    ...
    save_file = join(PLOTS_DIR, "HepG2_introns_intergenic_barchart_tall.pdf")
    print(save_file)
    plt.savefig(save_file)
...
def hots_prom_enh_barplot_in_legend(data=None):
    ...
    save_file = join(PLOTS_DIR, "HepG2_introns_intergenic_barchart_tall_in_legend.pdf")
    print(save_file)
    plt.savefig(save_file)
...
def plot_DHS_fractions():
    ...
    hot_en_non_dhs_count = hot_en.intersect(dhs, v=True).count()

    hot_pr_count = hot_pr.count()
    hot_pr_non_dhs_count = hot_pr.intersect(dhs, v=True).count()
    ...
    g1 = plt.bar([0, 1], dhs_percs, label="DHS")
    g2 = plt.bar([0, 1], non_dhs_percs, bottom=dhs_percs, label="non DHS")
